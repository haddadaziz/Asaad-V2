...
if (isset($_POST['btn_reserver'])) {
    ...
    $restantes = Visite::getPlacesRestantes($conn, $idvisite, $capacite_max);
    if ($nb_places <= $restantes) {
        Visite::ajouterReservation($conn, $idvisite, $idutilisateur, $nb_places);
        header("Location: visiteur.php?status=success_reservation#visites");
    } else {
        header("Location: visiteur.php?status=error_full#visites");
    }
    exit();
}

$id_user = $_SESSION['user_id'];
$visites_futures = Visite::find_visites_futures($conn, $q);
$historique = [];
...
<head>
    ...
    <script src="script.js?v4" defer></script>
    <link
    ...
    <section id="visites" class="py-20 bg-gray-50 relative">
        ...
        <div class="container mx-auto px-6 relative z-10">
            ...
            <div
                ...
                <form method="GET" action="#visites" class="flex flex-col md:flex-row items-center p-2 gap-2">
                    <input type="hidden" name="section" value="visites">

                    <div class="flex-grow w-full md:w-auto flex items-center px-4">
                        <i class="fa-solid fa-magnifying-glass text-gray-400 mr-3"></i>
                        <input type="text" name="q" placeholder="Rechercher une visite..."
                            class="w-full bg-transparent border-none focus:ring-0 text-gray-700 outline-none">
                    </div>
                    <button type="submit"
                        class="bg-maroc-red text-white font-bold py-3 px-8 rounded-full hover:bg-red-800 transition">
                        Rechercher
                    </button>
                </form>
            </div>
                    ...
                    <?php foreach ($visites_futures as $v): ?>
                        <div
                            ...
                            <div class="relative h-48 overflow-hidden bg-gray-200">
                                <img src="images/visiteguidee.png" alt="Visite"
                            ...
                            <div class="p-6 flex flex-col flex-grow">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <span class="text-gray-400 text-xs">
                                        ...
                                        <?= date('d/m/Y H:i', strtotime($v->getDateHeure())) ?>
                                    </span>
                                </div>

                                <h3 class="font-headings font-bold text-xl mb-2 text-gray-800">
                                    <?= $v->getTitre() ?>
                                </h3>

                                <div class="text-gray-500 text-sm mb-4 flex-grow">
                                    <p><i class="fa-regular fa-clock mr-2"></i>Durée : <?= $v->getDuree() ?> min</p>
                                    <p><i class="fa-solid fa-language mr-2"></i>Langue : <?= $v->getLangue() ?></p>
                                </div>

                                <div class="border-t border-gray-100 pt-4 mt-auto">
                                    <div class="flex items-center justify-between text-sm text-gray-600 mb-4">
                                        <span class="flex items-center gap-2">
                                            <i class="fa-solid fa-user-group text-maroc-green"></i> <?= $v->getCapacite() ?>
                                            places max
                                        </span>
                                    </div>

                                    <form method="POST" action="" class="flex gap-2">
                                        <input type="hidden" name="id_visite" value="<?= $v->getId() ?>">
                                        <input type="hidden" name="capacite_max" value="<?= $v->getCapacite() ?>">
                                        <input type="number" name="nb_places" value="1" min="1" max="20"
